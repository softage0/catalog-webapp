...
def fbconnect():
    ...
    url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_exchange_token=%s' % (config.FB_APP_ID, config.FB_APP_SECRET, access_token)  # noqa
    h = httplib2.Http()
...
def gconnect():
    ...
    try:
        ...
        oauth_flow = flow_from_clientsecrets('catalog/config/client_secrets.json', scope='')
